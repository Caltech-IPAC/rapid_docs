...
|--------------|----|------------------------------------------|
| [`glossary.md`](glossary) | DRAFT | The cross-cutting vocabulary defined once: l2file, grain, admission and the admission pointer, work unit, attempt, submission, batch execution, currency, dead letter, run, and the two run kinds, scratch (formerly campaign) and production, with owned and published work and the scratch purge |
...
| [`security.md`](security) | DRAFT | Reproducibility boundary and identifier exposure adopted; the nine service identities, including the owned-tier job role, re-derived against the target; no personal principal holds a delete action; human credential custody pending |
...
| [`storage.md`](storage) | DRAFT | Five-axis storage classification, bucket set and naming, enforced immutability (bucket policy, no Object Lock), promotion/swap mechanics, assurance; custody by run kind, owned custody with no delete fence and no copy path to published custody; DR posture open |
| [`catalog.md`](catalog) | DRAFT | Catalog core: promotion and product roles, delivery records, recoverability projection, restore acceptance; executor fence and product identity; promotion as release promotion, the dependency-closure rule for deletion |
| [`data-model.md`](data-model) | DRAFT | The data-model map: entity layers including the workflow and delivery layers, writers, relationships, the submission as an entity; the run entity with kind, owner, database target, configuration overlay and image digest, and its deletion semantics; the eleven cross-cutting invariants |
| [`operations.md`](operations) | DRAFT | Operational lifecycle, the controller, admission, result acceptance, reconciliation, association, alert production via the outbox, failure and problems paths, the `rapidctl` operator surface; no operator role, `run delete`, ceremony scoped to published work, the scratch purge with a pin, per-stage and per-subsystem entrypoints, the run-level database target |
